...
void data_aquisition(){
	...
	memcpy(data_buffer, send_time, size_time);
	memcpy(data_buffer+size_time, comma, 1);
	memcpy(data_buffer+1+size_time, send_q1, size_q1);
	memcpy(data_buffer+1+size_q1+size_time, comma, 1);
	memcpy(data_buffer+2+size_q1+size_time, send_q2, size_q2);
	memcpy(data_buffer+2+size_q1+size_time+size_q2, comma, 1);
	memcpy(data_buffer+3+size_q1+size_time+size_q2, send_torque, size_torque);
	memcpy(data_buffer+3+size_q1+size_time+size_q2+size_torque, endSymbol, 1);
